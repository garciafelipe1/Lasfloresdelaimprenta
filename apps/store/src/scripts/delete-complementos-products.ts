...
/**
 * Borra TODOS los productos que pertenezcan a la categoría "Complementos".
 *
 * DRY_RUN=true por defecto.
 * - DRY_RUN=true  -> lista
 * - DRY_RUN=false -> borra
 */
export default async function deleteComplementosProducts({ container }: ExecArgs) {
  ...
  if (!candidates.length) {
    logger.info("No se encontraron productos de Complementos para borrar.");
    return;
  }

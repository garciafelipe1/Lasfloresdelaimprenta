...
const TARGETS: Target[] = [
  ...
  { title: "Ramo San Valentín 1" },
  { title: "Ramo de San Valentín 2" },
];
 ...
 * - DRY_RUN=false -> borra
 */
export default async function deleteSvLegacyProducts({ container }: ExecArgs) {
  ...
  const targets = TARGETS.map((t) => ({
    ...
    handle: toHandle(t.title),
    skuPrefix: `${toHandle(t.title)}-`,
  }));

  logger.info(`Modo: ${dryRun ? "DRY_RUN (no borra)" : "BORRADO REAL"}`);
  logger.info("Objetivo (solo estos 3):");
  targets.forEach((t) =>

...
export default async function CheckoutSuccessPage(props: Props) {
  ...
  // El webhook debería completarlo automáticamente, pero hacemos un intento desde aquí como respaldo
    ...
    const attemptCartCompletion = async (cartId: string, retries = 3): Promise<boolean> => {
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          ...
          // Esperar un poco entre intentos (más tiempo en el primer intento para darle tiempo al webhook)
          const delay = attempt === 1 ? 2000 : 1000;
          ...
          try {
            const cartData = await medusa.store.cart.retrieve(cartId, {
              fields: 'payment_collection.authorized_amount,payment_collection.amount',
            });
            ...
            console.log(`[CheckoutSuccess] Estado del carrito (intento ${attempt}):`, {
              authorizedAmount,
              cartAmount,
              ready: authorizedAmount > 0,
            });
            
            if (authorizedAmount === 0) {
              console.warn(`[CheckoutSuccess] ⚠️ authorized_amount es 0, esperando...`);

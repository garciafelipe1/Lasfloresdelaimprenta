...
export default async function CheckoutSuccessPage(props: Props) {
  ...
  // El webhook también intentará completarlo, pero hacemos un intento desde aquí como respaldo
  if (external_reference && collection_status === 'approved' && status === 'approved') {
    console.log('[CheckoutSuccess] ✅ Pago aprobado, intentando completar carrito como respaldo...');
    
    try {
      // Esperar un poco para que el webhook tenga tiempo de procesar primero
      ...
      if (cartResponse?.type === 'order') {
        ...
        console.log('[CheckoutSuccess] El webhook completará el carrito, mostrando página de éxito');
      }
    } catch (completeError: any) {
      console.warn('[CheckoutSuccess] ⚠️ No se pudo completar carrito desde página de éxito (esperado si el webhook ya lo hizo)');
      console.warn('[CheckoutSuccess] Error:', completeError?.message);
      console.log('[CheckoutSuccess] Continuando para mostrar página de éxito...');
